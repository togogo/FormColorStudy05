...
public:
    ...
    Vec controller;//vector data of the controller
    ofVec2f location;//coordinate of vehicle
    Vec velocity;//velocity vector of vehicle, orientation uses velocity angle
    Vec acceleration;//acceleration vector added to velocity per frame
    Vec steer;//behavior steering vector
    Vec desired;//desired vector to steer to
    ...
    int contNum = 4;
    vector<ofVec2f> d;//alternate of: "Position d[] = new Position[contNum];"
    ofVec2f temp;//assign random initial value
    ofVec2f closest;//stores the id of the closest controller
    int placeHolder = 0;//store ID data
    
    void setup(ofVec2f _p,float _angle,float _magnitude, float _m, float _maxA, float _maxS, float _maxT);
    void update();
    Vec seek();
    void draw();
    void drawAllVectors();
